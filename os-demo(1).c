...
void os_demo_ip_cache_create_process(void)
{
    ...
    while (1) {
        ...
        if (msgQSend(g_msgQ_id, (char *)&msg, sizeof(msg), NO_WAIT, MSG_PRI_NORMAL) != 0) {
            ...
            // Remove node from linked list since message send failed
            semTake(g_list_sem, WAIT_FOREVER);
            if (g_ip_cache_head == node) {
                g_ip_cache_head = node->next;
            } else {
                ip_cache_node_t *current = g_ip_cache_head;
            ...
            // Free the node
            free(node);
            Print("Deleted node due to message send failure\n");
        }
