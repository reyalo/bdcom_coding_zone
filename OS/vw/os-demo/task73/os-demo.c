...
#define MAX_AGE 10 // Max age for cache nodes

typedef struct ip_cache_node {
    unsigned int ip;
    unsigned char mac[LEN_MAC];
    unsigned int age;
    unsigned int ref_count; // Reference count for node usage
...
void os_demo_ip_cache_create_process(void)
{
    ...
    while (1) {
        ...
        node->ref_count = 0; // Initialize reference count
        node->next = NULL;
        ...
        // Send message to DMMC with node pointer
        semTake(g_list_sem, WAIT_FOREVER); // Protect ref_count increment
        node->ref_count++; // Increment reference count
        ...
        if (msgQSend(g_msgQ_id, (char *)&msg, sizeof(msg), NO_WAIT, MSG_PRI_NORMAL) != 0) {
            ...
            node->ref_count--; // Decrement if send fails
        }
        semGive(g_list_sem);
    ...
    while (1) {
        if (msgQReceive(g_msgQ_id, (char *)&msg, sizeof(msg), WAIT_FOREVER) != ERROR) {
            ...
            if (msg.node) {
                ...
                msg.node->mac[4] = (unsigned char)(rand() % 256);
                msg.node->mac[5] = (unsigned char)(rand() % 256);
                ...
                msg.node->ref_count--; // Decrement reference count after processing
            } else {
...
void os_demo_ip_cache_age_check_process(void)
{
    while (1) {
        ...
        while (node) {
            ...
            if (node->age >= MAX_AGE && node->ref_count == 0) {
                if (prev) {
                    prev->next = node->next;
                } else {
