...
void os_demo_socket_text_process(void) {
	...
	message_t msg;
	 ...
	 UINT32 time_out = sysClkRateGet()/10;
    ...
    while (1)
    {
        ...
        if (eventReceive(ALL_EVENTS, EVENTS_WAIT_ANY, time_out, &receivedEvents) == OK)
        {
			int j=0;
            for (j = 0; j < 32; ++j)
            {
        ...
        int activity = so_select(max_fd + 1, &readfds, NULL, NULL, 0);
...
{
    UINT32 tid = taskNameToId(SERVER_TASK_NAME);
...
{	
	...
	Print("_______________Hello World_______________(2)\n");
