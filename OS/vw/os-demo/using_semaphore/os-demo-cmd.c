...
// #################################________msg_send___________________________________
...
static int second_os_demo_config_msg(int argc, char *argv[], struct user *u)
{
    ...
    return os_demo_send_msg(type, data);
...
// #################################__________event_send________________________________
...
static int second_os_demo_config_event(int argc, char *argv[], struct user *u)
{
    ...
    UINT32 mask = (UINT32)strtoul(argv[1], NULL, 0); //should be use bdcoms library

	return os_demo_send_event(mask);
