...
static int second_os_demo_config_test2(int argc, char **argv, struct user *u)
{
    ...
    char *taskNameToDelete = param.value.v_string;

    if (strncmp(taskNameToDelete, "DM", 2) != 0) {
        printf("Error: Task name must start with 'DM'\n");
        return -1;
    }

    int tid = (int)taskNameToId(taskNameToDelete);
    if (tid == 0) {
    ...
    if (taskDelete(tid) == 0) {
        printf("Success: Task '%s' (ID: 0x%X) deleted.\n", taskNameToDelete, tid);
    } else {
        printf("Error: Failed to delete task '%s'.\n", taskNameToDelete);
    }
