...
void os_demo_event_recv(void)
{
    ...
    while (1)
    {
		...
		/*if (eventReceive(0xFF, EVENTS_WAIT_ALL, WAIT_FOREVER, &receivedEvents) == OK)
        {
		...
		*/
        ...
        if (eventReceive(0xFFFFFFFF, EVENTS_WAIT_ALL, WAIT_FOREVER, &receivedEvents) == OK)
        {
            Print("Task DEVT, Received event(s): 0x%X\n", receivedEvents);
        }
        else
        {
            Print("eventReceive error\n");
        }
		sleep(1);
		if(i > 20)break;
    }
